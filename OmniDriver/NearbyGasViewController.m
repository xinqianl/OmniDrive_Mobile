...
//  OmniDriver
//
//  Created by Li Fang  on 7/31/16.
//  Copyright © 2016 Li Fang . All rights reserved.
//


#import "NearbyGasViewController.h"
...
#import "VBFPopFlatButton.h"
#import "RESideMenu.h"
@interface NearbyGasViewController ()
@property (nonatomic, strong) VBFPopFlatButton *leftButton;
@end

@implementation NearbyGasViewController
...
{
    self.curLoc = (CLLocation *)[locations lastObject];
    [self query];
}
-(void)addAllPins
{
    ...
    for (i = 0; i < [self.locs count]; i++) {
        [ name addObject:[NSString stringWithFormat:@"%@",[self.locs objectAtIndex:i][@"station_name"]]];
        [arrCoordinateStr addObject:[NSString stringWithFormat:@"%@, %@",[self.locs objectAtIndex:i][@"latitude"],[self.locs objectAtIndex:i][@"longitude"]]];
        [add addObject:[NSString stringWithFormat:@"%@, %@",[self.locs objectAtIndex:i][@"street_address"],[self.locs objectAtIndex:i][@"city"]]];
    }
...
- (UIButton *)carButton {
    UIImage *image = [UIImage imageNamed:@"gas.png"];
...
- (void) query{
    ...
    NSString *link = [[NSString alloc] initWithFormat:@"https://developer.nrel.gov/api/alt-fuel-stations/v1/nearest.json?api_key=4Z583BR0nbJiQKKGzhrjtUFD9cDZ697l3g2dw8XU&latitude=%f&longitude=%f&radius=5&limit=5", self.curLoc.coordinate.latitude, self.curLoc.coordinate.longitude];
    //    NSLog(link);
    ...
    NSURLSessionDataTask * dataTask = [defaultSession dataTaskWithURL:url
                                                    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                                                        if(error == nil)
                                                        {
                                                            ...
                                                            self.locs = [jsonResponse valueForKey:@"fuel_stations"];
